    ...
    enter: (node) => {
      switch (node.name) {
        ...
        case "StrikethroughMark": {
          if (lineIsActive(node.from)) break;
          // Only hide inline marks that live on a single line (skip fenced ```)
          if (node.name === "CodeMark" && node.to - node.from >= 3) break;
        ...
        case "ListMark": {
          ...
          if (mark === "-" || mark === "*" || mark === "+") {
            // A task item's ListMark is followed by a TaskMarker — keep bullet hidden then
            const after = state.doc.sliceString(node.to + 1, node.to + 2);
            if (after === "[") {
  ...
  class {
    ...
    build(view: EditorView): DecorationSet {
      ...
      for (const { from, to } of view.visibleRanges) {
        all.push(...computeLivePreviewDecorations(view.state, from, to));
      }
